...
if (!is_array($arParams["PROPERTY_CODES"])) {
	...
	// add sku properties
	$skuPropertyResult = CIBlockProperty::GetList(array(), array('IBLOCK_ID' => $arParams["IBLOCK_ID"], 'USER_TYPE' => 'SKU'));
	while ($skuProperty = $skuPropertyResult->GetNext()) {
		$arParams["PROPERTY_CODES"][] = $skuProperty['ID'];
	}
	
	// add CODE field
	$arParams["PROPERTY_CODES"][] = 'CODE';
}
...
if ($bAllowAccess) {
	...
	while ($arProperty = $rsIBLockPropertyList->GetNext())
	{
		...
		if ($arProperty["PROPERTY_TYPE"] == "E") {
			$arSelectE = Array("ID", "NAME");
			$arFilterE = Array( "IBLOCK_ID" => $arProperty['LINK_IBLOCK_ID'] , "ACTIVE_DATE" => "Y", "ACTIVE" => "Y");
			$dbAllElements = CIBlockElement::GetList(Array(), $arFilterE, false, false, $arSelectE);
	...
	$currencyResult = CCurrency::GetList(($by = "sort"), ($order1 = "asc"), LANGUAGE_ID);
	while ($currency = $currencyResult->Fetch()) {
		$arResult['CURRENCIES'][] = $currency;
	}
	//get prices
	$arResult['PRICES'] = array();
	$priceResult = CPrice::GetList(array('QUANTITY_FROM' => 'ASC'), array('PRODUCT_ID' => $arElement['ID']));
	while ($price = $priceResult->GetNext()) {
		$price["PRICE"] = CCurrencyLang::CurrencyFormat($price["PRICE"], $price["CURRENCY"], false);
		$arResult['PRICES'][] = $price;
	}
}
...
if ($bAllowAccess) {
	...
	if (check_bitrix_sessid() && (!empty($_REQUEST["iblock_submit"]) || !empty($_REQUEST["iblock_apply"])))
	{
				...
				if ($arResult["PROPERTY_LIST_FULL"][$propertyID]["PROPERTY_TYPE"] != "F")
				{
					...
					if ($arResult["PROPERTY_LIST_FULL"][$propertyID]["MULTIPLE"] == "Y")
					{
						...
						else
						{
							...
							{
								if (
									($arResult["PROPERTY_LIST_FULL"][$propertyID]["PROPERTY_TYPE"] == "L" || $arResult["PROPERTY_LIST_FULL"][$propertyID]["PROPERTY_TYPE"] == "E") && intval($value) > 0
									||
									$arResult["PROPERTY_LIST_FULL"][$propertyID]["PROPERTY_TYPE"] != "L" && $arResult["PROPERTY_LIST_FULL"][$propertyID]["PROPERTY_TYPE"] != "E" && !empty($value)
								)
								{
					...
					{
						if ($arResult["PROPERTY_LIST_FULL"][$propertyID]["PROPERTY_TYPE"] != "L" && $arResult["PROPERTY_LIST_FULL"][$propertyID]["PROPERTY_TYPE"] != "E")
		...
		foreach ($arParams["PROPERTY_CODES_REQUIRED"] as $key => $propertyID)
		{
			...
			//multiple property
			elseif ($arResult["PROPERTY_LIST_FULL"][$propertyID]["MULTIPLE"] == "Y" || $arResult["PROPERTY_LIST_FULL"][$propertyID]["PROPERTY_TYPE"] == "L" || $arResult["PROPERTY_LIST_FULL"][$propertyID]["PROPERTY_TYPE"] == "E")
			{
				if (is_array($propertyValue))
				{
					$bError = true;
		...
		if (empty($arResult["ERRORS"]))
		{
			...
			else
			{
				...
				if (!CCatalogProduct::Add($arFields)) {
					$arResult["ERRORS"][] = 'Ошибка создания товара';
				}
			...
			if (array_key_exists('PRICE', $_POST) && is_array($_POST['PRICE'])) {
				foreach ($_POST['PRICE'] as $keyPrice => $arPrice) {
					if ((int) $arPrice['QUANTITY_FROM'] == 0) {
						$arPrice['QUANTITY_FROM'] = false;
					}
					if ((int) $arPrice['QUANTITY_TO'] == 0) {
						$arPrice['QUANTITY_TO'] = false;
					}
					$priceAction = '';
					if (trim($arPrice['PRICE']) == '' && substr($keyPrice, 0, 5) != 'price') {
						$priceAction = 'delete';
					}
					if (trim($arPrice['PRICE']) != '' && substr($keyPrice, 0, 5) == 'price') {
						$priceAction = 'add';
					...
					switch ($priceAction) {
						case 'delete':
							CPrice::Delete($keyPrice);
							break;
						case 'add':
							CPrice::Add($arFields);
							break;
						case 'update':
							CPrice::Update($keyPrice, $arFields);
							break;
					}
				}
			}
