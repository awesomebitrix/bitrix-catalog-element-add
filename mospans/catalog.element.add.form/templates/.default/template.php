...
<form name="iblock_add" action="<?=POST_FORM_ACTION_URI?>" method="post" enctype="multipart/form-data">
	...
	<table class="data-table">
		...
		<tbody>
			...
			<?endforeach;?>
			<?if (array_key_exists('PRICES', $arResult) && array_key_exists('CURRENCIES', $arResult) /*&& (!empty($arResult['PRICES']) || $arParams['ID'] == 0)*/):?>
				<tr>
					<td>Цены</td>
					<td>
						<table class="prices">
						...
						<tbody class="prices__container">
						<?foreach ($arResult['PRICES'] as $priceItem):?>
							<tr>
								<td><input type="text" name="PRICE[<?=$priceItem['ID']?>][QUANTITY_FROM]" value="<?=$priceItem['QUANTITY_FROM']?>"></td>
								<td><input type="text" name="PRICE[<?=$priceItem['ID']?>][QUANTITY_TO]" value="<?=$priceItem['QUANTITY_TO']?>"></td>
								<td><input type="text" name="PRICE[<?=$priceItem['ID']?>][PRICE]" value="<?=$priceItem['PRICE']?>"></td>
								<td>
									<select name="PRICE[<?=$priceItem['ID']?>][CURRENCY]">
										<?foreach($arResult['CURRENCIES'] as $currencyItem):?>
										<option value="<?=$currencyItem['CURRENCY']?>" <?=($priceItem['CURRENCY'] == $currencyItem['CURRENCY']) ? 'selected': ''?>><?=$currencyItem['FULL_NAME'] . ' (' . $currencyItem['CURRENCY'] . ')'?></option>
										<?endforeach?>
									</select>
								</td>
							</tr>
						<?endforeach?>
							<tr>
							<?$newPriceCode = <<<HEREMARKER
								<td><input type="text" name="PRICE[#PRICE_ID#][QUANTITY_FROM]" value=""></td>
								<td><input type="text" name="PRICE[#PRICE_ID#][QUANTITY_TO]" value=""></td>
								<td><input type="text" name="PRICE[#PRICE_ID#][PRICE]" value=""></td>
								<td>
									<select name="PRICE[#PRICE_ID#][CURRENCY]">
HEREMARKER;
									foreach($arResult['CURRENCIES'] as $currencyItem) {
										$newPriceCode .= '<option value="' . $currencyItem['CURRENCY'] . '">' . $currencyItem['FULL_NAME'] . ' (' . $currencyItem['CURRENCY'] . ')' . '</option>';
									}
									$newPriceCode .= <<<HEREMARKER
									</select>
								</td>
HEREMARKER;
							echo str_replace('#PRICE_ID#', 'price_0', $newPriceCode);
							?>
							</tr>
						</tbody>
						</table>
						<button class="prices__adder">Добавить цену</button>
					</td>
				</tr>
			<?endif?>
...
<script>
	var newPriceInner = '<?=str_replace(array("\r", "\n"), array('', ''), $newPriceCode)?>';
</script>